...
for ilambda = 1:numel(lambda0)
    ...
    delt0 = [delt_shred*10,delt_shred*5,delt_shred*2];  % choose different delt
    ...
    for idtt = 1:numel(delt0)
        ...
        phi(1) = phitrue(1);
        for itt = 2:numel(tt)
            phi(itt) = phi(itt-1)/(1-gamma*delt);
        end
        plot(tt,real(phi),col{idtt},'linewidth',1);
    end
    xlabel('Time');ylabel('\phi');title(['\lambda = ',num2str(lambda)]);grid on;
    lg = legend('True','\Deltat = 10\Deltat(thres)','\Deltat = 5\Deltat(thres)',...
        '\Deltat = 2\Deltat(thres)','location','northwest');
